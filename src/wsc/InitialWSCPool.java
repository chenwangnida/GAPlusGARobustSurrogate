...
public class InitialWSCPool {
	...
	/**
	 * given all services associated with failure probability
	 *
	 * @param trucated normal distribution based on the mean and sd
	 *
	 *
	 */

	public void addFP2Sers(TruncatedNormal nd) {
		for (Service ser : this.swsPool.getServiceList()) {
			double fail_pr = nd.sample();

			boolean flag = true;
			while (flag) {
				if (fail_pr == 0 || fail_pr == 1) {
					fail_pr = nd.sample();
				} else {
					flag = false;
				}
			...
			ser.setFailure_probability(fail_pr);
			
			double  pr_succ = logScale4Success(fail_pr);
//			System.out.print(pr_succ);

			WSCInitializer.weight4NoFailrues += pr_succ;
