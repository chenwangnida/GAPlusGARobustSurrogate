...
public class GraphStatistics extends SimpleShortStatistics {
	...
	List<Double> meanFitness_testing = new ArrayList<Double>();
	List<Long> meanTime_testing = new ArrayList<Long>();
	List<Integer> fails_testing = new ArrayList<Integer>();
...
//            }
		...
		if (output) {
			...
			state.output.print("" + (popBestSoFar.fitness.fitness()) + " ", statisticslog); // best fitness of pop so
																							// far
			state.output.print("" + ((SequenceVectorIndividual) popBestSoFar).getFitness_value() + " ", statisticslog); // best
																														// fitness
																														// part1
																														// of
																														// pop
																														// so
																														// far
			state.output.print("" + ((SequenceVectorIndividual) popBestSoFar).getFitness_value2() + " ", statisticslog); // best
																															// fitness
																															// part2
																															// of
																															// pop
		...
		if (output) {
			...
			if (state.generation == state.parameters.getInt(new Parameter("generations"), null) - 1) {
                ...
                state.output.println("" + " " , statisticslog);               
				// Lets do the testing 4 the popBestSoFar
				testRobustness(popBestSoFar, state);
				
				// print the testing results in output.state file
				state.output.println(DoubleMath.mean(meanFitness_testing) + "", statisticslog);
				state.output.println(DoubleMath.mean(meanTime_testing) + "", statisticslog);
				state.output.println(DoubleMath.mean(fails_testing) + "", statisticslog);
	...
	// The main entry to the testing
	public void testRobustness(Individual popBestSoFar, EvolutionState state) {

		WSCInitializer.testingStartTime = System.currentTimeMillis();

		long initialization = System.currentTimeMillis() - WSCInitializer.testingStartTime;
		long startTime = System.currentTimeMillis();
		WSCInitializer.initTime_testing.add(initialization);

		// test the robustness

		// load the best individual objective
		SequenceVectorIndividual indi_best = (SequenceVectorIndividual) popBestSoFar;

		// calculate average fitness based on sampled disturbance
		for (int i = 0; i < 30; i++) {
			int countFailure = 0;

			long startTime4Loop = System.currentTimeMillis();

			WSCInitializer init = (WSCInitializer) state.initializer;

			init.random_disturbace = new Random(Long.valueOf(i));

			double avgFitness = calculateRustnessFitness(indi_best, state, init, countFailure, fails_testing);

			meanFitness_testing.add(avgFitness);

			meanTime_testing.add(System.currentTimeMillis() - startTime4Loop);

		}

		initialization = (long) 0.0;
		WSCInitializer.time_testing.add(System.currentTimeMillis() - startTime);
	}

	private double calculateRustnessFitness(SequenceVectorIndividual ind2, EvolutionState state, WSCInitializer init,
			int countFailure, List fails) {

		double f_sum = 0.0;
		for (int i = 0; i < init.robustNum_testing; i++) {
		...
		double part2 = f_sum / init.robustNum_testing;
//		ind2.setFitness_value2(part2);

//		System.out.println(countFailure);
		fails.add(countFailure);
